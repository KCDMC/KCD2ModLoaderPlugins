...
do
	...
	local getmetatable, setmetatable, type, traceback, getupvalue, setupvalue, upvaluejoin, getinfo, pack, unpack, error, pcall, xpcall, rawget, rawset, rawlen, tostring, pairs, ipairs, table, select
		= getmetatable, setmetatable, type, debug.traceback, debug.getupvalue, debug.setupvalue, debug.upvaluejoin, debug.getinfo, table.pack, table.unpack or unpack, error, pcall, xpcall, rawget, rawset, rawlen, tostring, pairs, ipairs, table, select

	if pack == nil then -- lua 5.1 fixes
		function pack(...)
			return { n = select('#', ...), ... }
		end
		
		local _next = next
		local next = function(o,k)
			local k = next(o,k)
			return k, o[k]
		end
		...
		function pairs(t,...)
			...
			local mpairs = meta and meta.__pairs
			if mpairs then return mpairs(t,...) end
			local mindex = meta and meta.__index
			if mindex then return next,t end
			return _pairs(t,...)
		end
	end
	...
	locals = export(function()
		...
		-- binds
		getmetatable, setmetatable, type, traceback, getupvalue, setupvalue, upvaluejoin, getinfo, pack, unpack, error, pcall, xpcall, rawget, rawset, rawlen, tostring, pairs, ipairs, table, select,
		rawtable,
		-- tables
	...
	exports = export(function()
		...
		-- global extensions
		table, next, rawnext, inext, rawinext, rawtostring, pairs, rawpairs, ipairs, rawipairs, qrawpairs, qrawipairs
	end)
...
for k,v in exports.pairs(exports) do
	public[k] = v
end
