...
--- still very broken on lua 5.1!

ModUtil = {}
ModUtil.Plugin = rom and rom.mods['Testing-ModUtil'] or ...
...
local _ENV = rom and rom.game or _G
setfenv(1,_ENV)

local error, pcall, xpcall = error, pcall, xpcall

local debug, type, table, select, unpack = debug, type, table, select, unpack or unpack
...
local function coroutine_running()
	local co = corunning()
	if co == nil then return comain end
end

local debug_getmetatable = debug.getmetatable

-- Extended Global Utilities (assuming lua ???)
...
-- unpack that respects metamethods
do
...
local function getEnv( thread )
	return threadEnvironments[ thread or coroutine_running( ) ] or _ENV_ORIGINAL
end
...
function ModUtil.Args.Join( args, ... )
	local args = ModUtil.Array.Join( args, pack( ... ) )
...
function ModUtil.String.Join( sep, ... )
	return table.rawconcat( pack( ... ), sep )
end
...
ModUtil.Metatables.Context = {
	__call = function( self, arg, callContext, ... )
		...
		contextInfo.args = pack( ... )
		contextInfo.arg = arg
		contextInfo.data = { }
		contextInfo.params = pack( objectData[ self ][ "prepContext" ]( contextInfo ) )
		
		threadEnvironments[ thread ] = contextInfo.env
		contextInfo.response = pack( contextInfo.wrap( unpack( contextInfo.params ) ) )
		threadEnvironments[ thread ] = penv

		if objectData[ self ][ "postCall" ] then
			contextInfo.final = pack( objectData[ self ][ "postCall" ]( contextInfo ) )
		end

		threadContexts[ thread ] = contextInfo.parent

		if contextInfo.final then
			return unpack( contextInfo.final )
		end
	end
}
...
ModUtil.Context.Call = ModUtil.Context(
	function( info )
		...
		return unpack( info.args )
	end,
	function( info )
		local thread = coroutine_running( )
		local penv = threadEnvironments[ thread ]
		threadEnvironments[ thread ] = info.env
		local ret = pack( info.arg( unpack( info.args ) ) )
		threadEnvironments[ thread ] = penv
		return unpack( ret )
	end
)
...
if ModUtil.Plugin then
	ModUtil.Plugin.mod = ModUtil
	ModUtil.Plugin.private.trigger_event.mod()
end